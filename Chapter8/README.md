...
在进行Web开发时，对于PHP程序来说最希望的就是能够把精力专注于后台服务的开发和实现上，而至于前端的展示页面则有美工和前端工程师来完成，但是很多事情都往往事与愿违，因为PHP代码是一种内嵌于HTML，并执行在Web服务器端的脚本语言，所以Web服务器会把PHP代码解析为HTML代码后进行合并输出，但这样的做法就会造成PHP代码混合在HTML之间，虽然已经有很多的基于MVC架构的库出现解决PHP开发中各种强耦合的问题，但是对于Web程序的输入、输出和具有逻辑特性的页面呈现都会让PHP代码和HTML代码之间产生很强的耦合性。为了解决这个问题，Andrei Zmievski和Monte Orte两位前辈从1999年就开始研究和编写解决该问题的方法和思路，于是乎就有了目前使用最为普遍，并且被PHP官方推荐的PHP模板引擎——Smarty，PHP模板引擎的出现使得PHP彻底和HTML进行了分离，使得PHP开发人员只需要通过简单的PHP函数就可以设置HTML中的动态数据，实现真正的MVC架构。TP本身支持PHP的原生模板引擎Smarty，也支持TP自己开发模板引擎Think，因为目前TP自己开发的模板引擎Think能够满足大部分的需求，因此本节也将以该模板为基础讲解模板引擎和视图的使用。
...
在Web开发中，很多时候一个页面会被重复使用，而只有其中的一个部分发生改动，在传统的开发中就需要为每个页面中重复编写相同的代码，这样的做法虽然可以实现最终页面的展示，但是对于页面修改和扩展却带来很多麻烦，因为开发人员需要把每个页面中重复的内容都进行修改，这样的做到既带来大量重复的工作，也使得这项工作非常容易出错，因此TP为了解决这个问题，提出了模板继承的概念。所谓模板继承，就是指开发人员定义一个基础模板，并且在其中通过关键字block定义需要在子模板中被重写的区域，然后当子模板通过关键字extend继承该基础模板时，开发人员只需要在子模板中对基础模板中的某些block区域进行重写即可。这样的做法的好处是，开发人员只需要把相同的部分编写一次，而在其他模板文件中只需要编写个性的东西，免去了相同部分重复编写的问题，并且当基础模板发生改变时也只需要修改一个文件就可以实现所有模板发生改动。

基础模板的编写和普通的HTML文件在大部分内容上没有任何区别，而区别在于需要在模板文件中定义多个由{block}{/block}标签组成的区块，具体代码如下：
...
<body>
    <p>基础模板的内容</p>
    {block name="main"}基础模板的主内容{/block}
    <br />
    {block name="footer"}基础模板的底部内容{/block}
</body>
</html>
```

在上面的代码中block标签定义了需要在子模板中被重写的区域，每个block标签都必须指定一个name属性用来标识当前区块的名称，并且这个name属性值必须在当前模板中是唯一的。block标签的使用非常灵活，其中可以包含任何模板内容，包括变量、条件判断标签等。在定义完基础模板之后，就可以定义子模板（即控制器的视图），这里以Index模块下的Index控制器中的index函数的视图文件index.html为例，具体代码如下：
...
在上面的代码中首先看到使用extend关键字标签定义了子模板继承于基础模板“base.html”，这里需要注意的是如果没有特别指定，那么这里的基础模板“base.html”默认就位于Index模块下view目录下。在子模板中，可以通过重新定义和基础模板中具有相同name属性值的block标签来告诉TP模板渲染器对该部分要进行重新渲染，当然也可以在基础模板中不重新定义具有相同name属性值的block标签，那么此时TP就会使用子模板进行重新渲染，而是沿用基础模板中block标签所定义的内容，而如果定义了一个和基础模板相同的block标签，但是确在该标签中没有添加任何东西，那么就表示删除基础模板中的该区块内容。此外，在上面子模板中还有一个{\_\_block\_\_}标签，该标签的作用是引用原先基础模板中内容到{\_\_block\_\_}标签处，然后和子模板中该区域的其他内容进行合并，得到新的内容，结果如图8-18所示。

![templet_view](Screenshot/templet_view.png)

图8-18 模板基层的效果

这里还有一点需要注意的是，在当前子模板继承了基础模板，那么子模板中只能在block标签的定义区类编写相应代码，而不能除block标签之外的地方添加代码，如果添加了TP渲染时也会把这些代码直接忽略，即不显示在最终的页面中，示例代码如下：
...
通过本章的学习，了解了基于MVC架构的ThinkPHP的基本使用方法，并且深入分析了ThinkPHP中的控制器、模型、视图、路由等非常重要的概念。本章首先深入分析了MVC的一些基本特性，以及常用的体系结构，并且了解了MVC中各模块之间的相互作用，帮助开发人员对于ThinkPHP的体系结构有一个基本的认识，为后续的学习提供了理论基础。其次详细讲解了ThinkPHP的整个部署过程，以及ThinkPHP中各个文件夹的作用，通过这部分的学习可以让开人员能够了解在编写代码时的相应编写位置，同时体会ThinkPHP在设计时是如何与MVC的思想进行结合。随后使用大量篇幅解了ThinkPHP中非常重要的控制器，因为控制器是所有操作的路口，同时还讲了如何通过控制器接收各种HTTP请求，通过这部分的学习就可以让开发人员知道如何向ThinkPHP发送的请求，以及在ThinkPHP中如何解析这些请求。然后，本章还讲了ThinkPHP中最为核心，也是业务流程中最为核心的模型，以及数据库的ORM，通过这部分的讲解，开发人员可以掌握如果通过模型实现业务，并且通过分层的方式把业务分为不同的层次。最后讲了ThinkPHP中的内置模板引擎，通过模板引擎就可以实现HTML文件和控制器之间的绑定，并且通过模板的之间继承使得开发页面的效率大为提高。但是本章的内容实际上还有很多可以深入的方法，例如使用软件工程的概念设置业务模型、通过ThinkPHP提供的验证器来实现Web中各类信息的验证，以及如何在ThinkPHP中借助PHP的GD库实现验证码图片等，这些对于开发人员来说可以在后续的使用中逐步研究。